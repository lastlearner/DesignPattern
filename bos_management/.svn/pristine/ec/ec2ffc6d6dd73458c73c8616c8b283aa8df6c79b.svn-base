			...
			$(function() {
				...
				*/
				...
				$("#btnEp").click(function(){
					...
					var ret = $("#editPwdForm").form("validate");
					if(ret){
						$("#editPwdForm").submit();
					}else{
						alert("您两次输入的密码不正确 , 请重试!");
					}
				});
	...
	<body class="easyui-layout">
		<div data-options="region:'north',border:false" style="height:70px;padding:10px;">
			...
			<div id="sessionInfoDiv" style="position: absolute;right: 5px;top:10px;">
				[<strong> ${sessionScope.loginUser.username } </strong>]，欢迎你！您使用[<strong> <%=request.getRemoteAddr()%></strong>]IP登录！
			</div>
        ...
        background: #fafafa">
			<div class="easyui-layout" fit="true">
				...
				<form id="editPwdForm" action="${pageContext.request.contextPath }/userAction_editPwd.action" method="post">
				<input type="hidden" name="username" value="${sessionScope.loginUser.username }">
					<table cellpadding=3>
						<tr>
							<td>新密码：</td>
							<td>
								<input id="txtNewPass" name="password" type="password" class="easyui-validatebox" data-options="required:true"/>
							</td>
						</tr>
						<tr>
							<td>确认密码：</td>
							<td>
								<input id="txtRePass" name="repassword" type="password" class="easyui-validatebox" required="required" validType="equals['#txtNewPass']"/>
								<script type="text/javascript">
									$(function(){
										//扩展方法.判断两次输入的值是否一样
